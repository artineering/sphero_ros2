...
* Dynamics integrate accel->velocity->position and gyro(z)->yaw, so a missing
  camera blob => PREDICT-ONLY that cycle (the filter dead-reckons) and we STILL
  publish.

Body->field rotation
--------------------
Telemetry velocity/accel are body-frame; they are rotated into the field frame
using the filter's current yaw estimate (the robot's field heading). Compass
calibration makes telemetry yaw magnetic-north-referenced across all robots, so a
single global constant `body_to_field_yaw_offset` (angle magnetic-north -> field
+x) maps telemetry yaw into the field-frame heading; that field heading is what
rotates the body vectors. Keeping the rotation on the MEASUREMENT keeps H/R
constant and the filter linear.
"""
